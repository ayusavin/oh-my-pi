...
| `advisor.display`     | enum    | `all`   | How advisor notes render in the transcript: `all`, `blockers` (blocker-severity notes only), or `none` (never render a card). Does not change what the advisor runs or what reaches the agent's context — only the transcript card. |
...
| `tools.collapsedPreviewLines`  | number  | `-1`    | Lines of tool output shown while a tool card is collapsed (bash, eval, read, grep, edit, and other tool previews). `-1` keeps the terminal-height default; `0` hides output until expanded (ctrl+o); `N` shows exactly `N` lines. Never affects an already-expanded card.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `display.toolCalls`            | enum    | `full`  | How tool calls render in the transcript. `full` keeps the standard card; `compact` collapses each call to one line (intent, outcome, size, duration) with no frame; `grouped` additionally folds consecutive calls in a turn into one summary row. ctrl+o steps collapsed → per-call lines → the full card, which is unchanged from `full`. |
| `display.expandScope`          | enum    | `session` | What ctrl+o expands. `session` flips one flag for the whole transcript, so every live tool block changes at once. `block` expands only the selected block: the cursor starts at the newest tool block, `alt+k` / `alt+j` move it, and inside an expanded group the same keys move between that group's calls. Blocks already retired into terminal scrollback cannot change under either scope. |
