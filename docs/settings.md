...
| `mem0.agentId`                | string  | `omp`                                    | Mem0 `agent_id` namespace for this agent. Every recall, list and scope check filters on it, so memories written by other agents under the same Mem0 user are never returned. |
| `mem0.appId`                  | string  | `omp`                                    | Mem0 `app_id` namespace for this installation; written on every memory and required on every read. |
| `mem0.profileRecallLimit`     | number  | `8`                                      | Standing preferences recalled per prompt by relevance to the prompt text (1-32). The full profile is never injected wholesale, so a large preference set cannot overflow the injection budget. |
